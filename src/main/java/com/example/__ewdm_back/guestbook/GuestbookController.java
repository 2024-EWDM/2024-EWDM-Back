...
public class GuestbookController {
    ...
    @PutMapping(value = "/guestbook/{postId}/update", consumes = {"multipart/form-data"})
    public ResponseEntity<?> updateGuestbook(@PathVariable Integer postId, @RequestPart(name="data") GuestBookPutRequest guestBookPutRequest, @RequestPart(name="file") List<MultipartFile> multipartFilelist) {
        List<Image> images;
        Guestbook guestbook = guestbookService.updateGuestbook(postId, guestBookPutRequest);
        if (guestbook == null) return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("게시글 생성에 실패했습니다.");

        if (multipartFilelist != null) {
            images = s3Service.updateGuestbook(guestbook, multipartFilelist);
            if (images == null) return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("사진 등록에 실패했습니다.");
        }
    ...
    @DeleteMapping("/guestbook/{postId}/delete")
    public ResponseEntity<?> deleteGuestbook(@PathVariable Integer postId) {
        Guestbook deleted = guestbookService.deleteGuestbook(postId);
        return (deleted!=null)?
                ResponseEntity.status(HttpStatus.OK).build():
                ResponseEntity.status(HttpStatus.BAD_REQUEST).body("삭제하려는 게시글이 없습니다.");
    }
}
